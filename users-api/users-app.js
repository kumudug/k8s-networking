...
app.post('/signup', async (req, res) => {
  ...
  try {
    const hashedPW = await axios.get(`http://${process.env.CORE_DNS_ADDRESS}/hashed-password/` + password);
...
app.post('/login', async (req, res) => {
  ...
  const response = await axios.get(
    `http://${process.env.CORE_DNS_ADDRESS}/token/` + hashedPassword + '/' + password
  );
